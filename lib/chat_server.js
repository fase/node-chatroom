...
exports.listen = function(server) {
	// Start Socket.IO server.  Allow it to piggyback on existing http server.
	...
	io.sockets.on('connection', function(socket) {
		...
		// Handle user messages, change-change attempts & room creation/changes.
        ...
        socket.on('users', function() {
            ...
            var users = io.of('/' + currentRoom[socket.id]).clients();

            for(var i = 0; i < users.length; i++) {
                usersInRoom.push(nickNames[users[i].id]);
            }
...
function joinRoom(socket, room) {
	...
	// Note that the user is now in this room.
	currentRoom[socket.id] = room;

	// Let user know they're now in a new room.
	socket.emit('joinResult', {room: room});
	
	// Let other users in the room know that user has joined.
